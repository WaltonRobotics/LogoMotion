...
//NOTE Paul has implemented these jags in main. Making a copy of them in here is a bad         idea. I would say to pass them but then that would be circumventing Robotmain. I’ll come         back to this
...
//Will come back to this but the distance moved isn’t going to register this way. It’ll blaze through the code before the jag has moved the forklift
    public void forkliftControl(double speedToGo) {
        forkliftJag.set(speedToGo);
        if (speedToGo < 0) {
            currentHeight -= forkliftEncoder.getDistance();
        } else {
            currentHeight += forkliftEncoder.getDistance();
        }
...
//    }
    ...
    public GoToHeight getNewGoToHeightInstance(int height) {
